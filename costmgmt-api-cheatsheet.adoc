...
Per-organization sizing and classification thresholds for container, namespace, node, GPU, and PVC plugins. OpenShift Virtualization (VM) thresholds are configured via `/settings/vm` (see <<OpenShift Virtualization (VM) recommendations>>), not this endpoint.

The required query parameter `recommendation_type` accepts `container`, `namespace`, `node`, `gpu`, or `pvc`. PUT accepts partial updates; DELETE removes tenant overrides so environment and compiled defaults apply. Fields locked by environment variables are listed in `locked_fields` and return `403` if modified via the API.
